...
var AppealService = (function() {
  ...
  AppealService.prototype.loadAppeals = function() {
    var _this = this;
    this.http
      .get(this._appealUrl + '?' + this._populateCampaign)
  ...
  AppealService.prototype.filterAppeals = function(filters) {
    if (filters.campaign) {
      var url =
        this._appealUrl + '?' + this._populateCampaign + '&query={"info.campaign":"' + filters.campaign._id + '"}';
  ...
  AppealService = __decorate([core_1.Injectable(), __metadata('design:paramtypes', [http_1.Http])], AppealService);
